...
export class LoginComponent implements OnInit {
    ...
    constructor(private activeRoute: ActivatedRoute, private router: RouterExtensions, private apiService: ApiService) { }

    onSubmit() {

        console.log("Inizio il login");
        ...
        this.apiService.login(body.toString()).subscribe((data) => {
            ...
            this.apiService.setToken(JSON.stringify(data).toString());
        ...
        if (isAndroid) {
            this.textfields.forEach(
                (item) => {
                    console.log("ITEM");
                    console.log(item);
                    item.nativeElement.dismissSoftInput();
                }
            );
        }
    }
